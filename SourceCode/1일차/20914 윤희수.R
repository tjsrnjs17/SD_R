...
텍스트파일자체<-file("잠이 오질 않네요.txt", encoding = "UTF-8") #노래.txt파일 utf-8 형식으로 불러오기
텍스트파일내용 <- readLines(텍스트파일자체) #줄 별로 읽기
close(텍스트파일자체)

명사만 <-sapply(텍스트파일내용, extractNoun, USE.NAMES = F) #명사만 읽기
명사만 <- unlist(명사만) #데이터 벡터화

head(명사만, 30) #30개만 출력

중복횟수 <- table(명사만)
#위의 '명사만'이라는 변수(데이터)를
#같은 단어를 기준으로 합쳐서 몇 번 중복됐는지 계산

head(sort(중복횟수, decreasing = T), 30)#빈도 수가 많은 순으로 30개 출력
